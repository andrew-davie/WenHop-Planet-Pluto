...
void drawPlanet(int half) {
    ...
    for (absScanline = startScanline; absScanline < SCANLINES && line85[equiv] >= 0;) {
        
        xchar = RAM + _BOARD + (half + frac) + boardWidth * (line85[equiv] >> 5);
...
#define PUT2(i) \
    ...
    while (absScanline < lastBottom && absScanline < SCANLINES) {
        *pf0++ = 0;
        *pf1++ = 0;
        *pf2++ = 0;
        absScanline++;
