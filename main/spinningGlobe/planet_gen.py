#!/usr/bin/env python3
"""
planet_gen_all.py — Random planet texture map generator
...
Usage:
    python planet_gen_all.py                         # one random planet
    python planet_gen_all.py -n 9                    # nine random planets
    python planet_gen_all.py -t terran               # force a type
    python planet_gen_all.py -t lava -s 1234         # seeded for reproducibility
    python planet_gen_all.py -n 20 -o ./textures     # batch to a folder
    python planet_gen_all.py --all                   # one of every type
...
W, H = 1024, 512      # 2:1 equirectangular
...
def snoise(sigma):
    ...
    scipy mode sequence = (axis-0/rows/Y, axis-1/cols/X).
    We want Y to reflect (poles) and X to wrap (left-right seam).
    """
    r = np.random.randn(H, W).astype(np.float32)
    b = gaussian_filter(r, sigma=sigma, mode=('reflect', 'wrap'))   # Y=reflect, X=wrap
    lo, hi = b.min(), b.max()
...
def ice_poles(rgb, extent, strength=0.88):
    ...
    y   = np.linspace(0, 1, H, dtype=np.float32)[:, np.newaxis, np.newaxis]
    t   = np.maximum(
...
def terran():
    ...
    stops = [
        ...
        (wl*0.85,    [ 21,  68, 150]),
        (wl,         [ 44, 103, 170]),
        (wl+lr*0.05, [190, 170, 120]),
        (wl+lr*0.22, [ 70, 116,  46]),
        (wl+lr*0.52, [ 50,  86,  30]),
        (wl+lr*0.74, [ 92,  78,  58]),
        (1.00,       [218, 220, 226]),
    ]
...
def gas_banded(style):
    warp = (snoise(45) - 0.5) * H * random.uniform(0.06, 0.14)
    ...
    pals = {
        'jupiter': [
            (0.00,[191,120, 71]),(0.20,[226,172,116]),(0.40,[200,146, 86]),
            (0.60,[236,196,150]),(0.80,[170, 91, 50]),(1.00,[210,160,106])],
        'saturn':  [
            (0.00,[190,170,116]),(0.25,[222,200,150]),(0.50,[202,180,126]),
            (0.75,[240,226,180]),(1.00,[180,156,100])],
        'ember':   [
            (0.00,[163, 50, 20]),(0.25,[210,110, 50]),(0.50,[190, 70, 30]),
            (0.75,[230,150, 80]),(1.00,[148, 40, 10])],
    }
...
def main():
    ...
    ap.add_argument('-n', '--count',  type=int, default=1,      help='Planets to generate')
    ap.add_argument('-t', '--type',   choices=list(TYPES),      help='Force planet type')
    ap.add_argument('-s', '--seed',   type=int,                 help='RNG seed')
    ap.add_argument('-o', '--output', default='.',              help='Output directory')
    ap.add_argument('--all',          action='store_true',      help='One of every type')
    args = ap.parse_args()
    ...
    for i, (seed, ptype) in enumerate(jobs):
        ...
        print(f'[{i+1}/{len(jobs)}]  {ptype_out:14s}  seed={seed:06x}  →  {fpath}')
