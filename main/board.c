...
// _untimed_ = no measurement yet. _nop_ = confirmed no handler in either processTypes()'s
// or processCreatures()'s switch (board.c), so it can never do real work here -- distinct from
// _untimed_ specifically so a grep for _untimed_ only turns up characters still awaiting a real
...
// Last updated: 2026-08-07 20:47 AEST
static const unsigned short budget[128] = {
    ...
    _B + 10059,    //  19 CH_MELLON_HUSK (avg: 1698) -- updated 2026-08-07 01:41 AEST (was 5000)
    ...
    _B + 3393,     //  52 CH_GEODOGE_FALLING (avg: 374) -- updated 2026-08-07 20:37 AEST (was 1846)
    ...
    case TYPE_BOMB:
        ...
        if (*Animate[type] == CH_BLANK) {
            *cur->me = CH_BLANK;
            ...
            nDots(1, cur->col, cur->row, PT_SPIRAL, 10, 4, 1, rangeRandom(50) + 20, 1 + (getRandom32() & 2));
        break;
