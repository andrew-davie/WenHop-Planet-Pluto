...
// GS_RASTER_BLEED disabled -- nothing in the game transitions into this state any more, and the
// four functions below are left as empty stubs (rather than deleted) purely because main.c's
// init/kernel/VB/OS dispatch tables are positional arrays indexed by enum GAME_STATE, so the
// GS_RASTER_BLEED slot in each still needs a valid function pointer to keep every other state's
// index aligned. Bodies commented out below so nothing runs even if this state were ever reached.
