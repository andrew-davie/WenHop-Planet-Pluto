 ...
 * lena_screen_frames, a table of 3 pointers in cycle order. Index it
 * by a running frame counter (mod LENA_NUM_FRAMES) exactly as usual --
...
#define LENA_PARAMS "image=lena.jpg width=48 height=128 sigma=1.0 radius=1 rounds=10 sweeps=3 metric=luma seeds=5 smoothness=0.5 swaps=False saturation=1.0 brightness=1.0 frames=3 temporal_weight=0.0 frame_scheme=cumulative frame_mode=cyclic outer_rounds=10 colour_select=dp diffusion_gain=1.0 roll_scanlines=True roll_band_height=16"

/* e.g. lena_screen_frames[frame_counter % LENA_NUM_FRAMES] */
extern const ScanLine * const lena_screen_frames[LENA_NUM_FRAMES];
