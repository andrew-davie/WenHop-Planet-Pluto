#ifndef PARROT_H
#define PARROT_H

#include <stdint.h>

/* 3-frame vertically-blended scanline data.
 * Generated by atari_scanline_blend.py -- do not hand-edit.
 *
 * Screen is SCREEN_WIDTH x SCREEN_HEIGHT (48x128). Each ScanLine
 * holds one Atari colour byte (COLUP0/COLUP1/COLUBK register format:
 * bits 7-4 = hue, bits 3-1 = luminance, bit 0 = 0) and a SCREEN_WIDTH-bit
 * on/off bitmap (packed MSB-first, column 0 = bit 7 of bits[0]).
 *
 * ScanLine is defined once and shared verbatim across every file this
 * tool generates. The 3 individual frame arrays are file-local
 * (static) -- the only thing this translation unit exports is
 * parrot_screen_frames, a table of 3 pointers in cycle order. Index it
 * by a running frame counter (mod PARROT_NUM_FRAMES): cycle frame 0, 1,
 * ..., 2, back to 0, forever. Only frame0 was solved to look
 * reasonable on its own; every later frame corrects whatever residual
 * error the frames before it left behind, so the 1/3-weighted
 * temporal average lands closer to the source image than fewer frames
 * could manage. Do not display any frame after frame0 on its own
 * expecting it to look like the picture -- each is a correction term,
 * not a picture in its own right.
 */

#define PARROT_NUM_FRAMES 3

#ifndef ATARI_SCANLINE_TYPE_H
#define ATARI_SCANLINE_TYPE_H

#define SCREEN_WIDTH  48
#define SCREEN_HEIGHT 128
#define SCREEN_BYTES_PER_ROW 6

typedef struct {
    uint8_t colour;
    uint8_t bits[SCREEN_BYTES_PER_ROW];
} ScanLine;

#endif /* ATARI_SCANLINE_TYPE_H */

#define PARROT_PARAMS "image=parrot.jpg width=48 height=128 sigma=1.0 radius=5 rounds=10 sweeps=3 metric=luma seeds=6 smoothness=0.2 swaps=False saturation=1.5 brightness=1.5 frames=3 temporal_weight=0.0 frame_scheme=cumulative frame_mode=cyclic outer_rounds=12 roll_scanlines=False roll_band_height=None"

/* e.g. parrot_screen_frames[frame_counter % PARROT_NUM_FRAMES] */
extern const ScanLine * const parrot_screen_frames[PARROT_NUM_FRAMES];

#endif /* PARROT_H */
