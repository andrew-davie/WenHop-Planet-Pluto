...
/* 5-frame vertically-blended scanline data.
 ...
 * tool generates. The 5 individual frame arrays are file-local
 * (static) -- the only thing this translation unit exports is
 * dh_screen_frames, a table of 5 pointers in cycle order. Index it
 * by a running frame counter (mod DH_NUM_FRAMES) exactly as usual --
 * the display-side code does not change at all versus the unrolled
 * scheme.
 *
 * IMPORTANT: --roll-scanlines was used (band height 40), so
 * these 5 arrays are NOT the raw per-frame solves -- they are
 * already-rolled composites. Array k's row y holds original solved frame
 * (k + y // 40) %% 5's row y data: every
 * 40-row BAND of the screen is staggered to a different
 * original frame, instead of the whole screen flipping between frames in
 * lockstep. Over one full 5-frame cycle every row still visits
 * all 5 original frames' data once each (same 1/5-weighted
...
#define DH_PARAMS "image=dh.jpg width=48 height=128 sigma=1.0 radius=5 rounds=8 sweeps=3 metric=luma seeds=6 smoothness=0.5 saturation=1.0 brightness=0.8 frames=5 outer_rounds=8 temporal_weight=0.0 roll_scanlines=True roll_band_height=40 frame_scheme=cumulative"
